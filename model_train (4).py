...
def main():
    ...
    total_steps = hyper_params.epoch_num * len(train_loader)
    compressor_scheduler = CosineAnnealingLR(compressor_opt, T_max=total_steps, eta_min=1e-6)
    estimator_scheduler = CosineAnnealingLR(estimator_opt, T_max=total_steps, eta_min=1e-6)
    ...
    for i in range(hyper_params.epoch_num):
        for x, _ in train_loader:
            ...
            compressor_opt.step()
            estimator_opt.step()
            compressor_scheduler.step()
            estimator_scheduler.step()
